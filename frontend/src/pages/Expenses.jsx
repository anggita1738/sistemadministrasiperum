...
export default function Expenses() {
    ...
    const fetchMonthlyDetail = () => {
        api.get(`/reports/monthly?month=${selectedMonth}`).then((res) =>
    ...
    const expenseHeaders = ["Tanggal", "Keterangan", "Jumlah"];
    const expenseRows = expenses.map((expense) => [
        expense.expense_date,
        expense.description,
        Number(expense.amount || 0),
    ]);
    const expenseRowsDisplay = expenses.map((expense) => [
    ...
    const handleExportExcel = () => {
        downloadCsv("data-pengeluaran.csv", expenseHeaders, expenseRows);
    };

    const handleExportPdf = () => {
        downloadPdf({
            title: "Data Pengeluaran",
            filename: "data-pengeluaran.pdf",
            headers: expenseHeaders,
            rows: expenseRowsDisplay,
        });
    };
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
                        <h3 className="font-bold text-slate-800">
                            Laporan Bulan: <span className="print-only">{selectedMonth}</span>
                        </h3>
                        <input
                            type="month"
                            value={selectedMonth}
                            onChange={(e) => setSelectedMonth(e.target.value)}
                            className="border border-slate-300 rounded-xl px-4 py-2 outline-none focus:ring-2 focus:ring-blue-500 no-print"
                        />
                    </div>
                    ...
                    {monthlyDetail && (
                        <div className="glass bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                            ...
                            <div className="pt-4 border-t border-slate-100">
                                ...
                                <p
                                    className={`text-3xl font-bold ${monthlyDetail.total_income - monthlyDetail.total_expense >= 0 ? "text-blue-600" : "text-red-600"}`}
                                >
                                    Rp{" "}
                                    {(
                                        monthlyDetail.total_income -
                ...
                <div className="space-y-4">
                    <div className="space-y-4">
                        ...
                        <div>
                            <button
                                onClick={openModal}
                                className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-xl text-sm font-medium shadow-sm transition-colors"
                            >
                                + Catat Pengeluaran
                            </button>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full border border-slate-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-red-500 outline-none"
                                />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">
                            ...
                            <div className="flex justify-end space-x-3 mt-8">
                                ...
                                <button
                                    type="submit"
                                    className="px-5 py-2 rounded-xl bg-red-500 text-white hover:bg-red-600 font-medium shadow-sm transition-colors"
                                >
                                    Simpan
                                </button>
                            </div>
